...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<HttpRes>
) {
  if (req.method !== "POST") {
    res
      .status(StatusCode.MethodNotAllowed)
      .send(new HttpRes(StatusCode.MethodNotAllowed, "Method not allowed"));
  }
  ...
  try {
    ...
    res
      ...
      .send(new HttpRes(StatusCode.OK, "Message has been sent successfully"));
  } catch (err) {
